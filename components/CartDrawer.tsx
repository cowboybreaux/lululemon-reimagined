...
export default function CartDrawer() {
  ...
  return <div className={`cart-layer ${isOpen ? "open" : ""}`} aria-hidden={!isOpen}>
    ...
    <aside ref={panel} role="dialog" aria-modal={isOpen ? true : undefined} aria-labelledby="cart-title" className="drawer">
      ...
      <div className="items px-6 sm:px-8">
        {!items.length ? <p className="py-12 text-lg opacity-70">Your bag is empty.</p> : <ul>
          {items.map(item => <li key={item.key} className="flex gap-4 border-b border-[#170306]/20 py-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm"><span>Qty: {item.quantity}</span><button type="button" className="underline underline-offset-4" aria-label={`Remove ${item.name}${item.colour ? ` — ${item.colour}` : ""}, size ${item.size}`} onClick={() => { close.current?.focus({ preventScroll: true }); removeItem(item.key); }}>Remove</button></div>
            </div>
          </li>)}
        </ul>}
      </div>
    ...
    <style jsx>{`
      .cart-layer { position: fixed; inset: 0; z-index: 100; visibility: hidden; pointer-events: none; transition: visibility 0s 420ms; }
      .cart-layer.open { visibility: visible; pointer-events: auto; transition-delay: 0s; }
      .backdrop { position: absolute; inset: 0; background: rgba(23,3,6,.25); opacity: 0; transition: opacity 420ms ease; }
      .open .backdrop { opacity: 1; }
      .drawer { position: absolute; inset: 0 0 0 auto; width: min(100%, 480px); height: 100%; background: #fffffa; color: #170306; display: flex; flex-direction: column; transform: translateX(100%); transition: transform 420ms cubic-bezier(.22,1,.36,1); }
      .open .drawer { transform: translateX(0); }
      .items { overflow-y: auto; overscroll-behavior: contain; flex: 1; min-height: 0; }
