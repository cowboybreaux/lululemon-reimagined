  ...
  return <div className={`quick-add mt-3 ${dark ? "on-dark" : ""}`}>
    ...
    <div id={id} aria-hidden={!expanded} className={`reveal ${expanded ? "expanded" : ""}`}>
      <div className="min-h-0 overflow-hidden">
        ...
        <div className={`reveal ${size && expanded ? "expanded" : ""}`} aria-hidden={!size || !expanded}><div className="min-h-0 overflow-hidden"><button type="button" tabIndex={active && expanded && size ? 0 : -1} disabled={!size || !expanded || !active} className="add mt-3 border px-3 sm:px-5 py-3 text-xs font-bold tracking-[0.15em]" onClick={() => { if (active && expanded && size) { addItem(product, size); setAdded(true); } }}>ADD TO CART</button></div></div>
      </div>
    </div>
    <div className={`reveal ${added ? "expanded" : ""}`}><div className="min-h-0 overflow-hidden"><span aria-hidden="true" className="block mt-2 text-xs opacity-70">Added to cart</span></div></div>
